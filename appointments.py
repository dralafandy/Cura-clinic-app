        ...
        with st.form("book_appointment_form"):
            ...
            with col1:
                ...
                with st.expander("➕ إضافة مريض جديد سريع"):
                    ...
                    if st.form_submit_button("حفظ المريض الجديد"):
...
def apply_appointments_filters(appointments_df, date_filter, doctor_filter, status_filter):
...
def display_appointments_cards(appointments_df):
    ...
    # ترتيب حسب الوقت
    appointments_df = appointments_df.sort_values('appointment_time')
        ...
        with st.container():
            ...
            with col4:
                ...
                if appointment['status'] == 'مجدول':
                    if st.button(f"✅", key=f"confirm_{appointment['id']}", help="تأكيد"):
                        crud.update_appointment_status(appointment['id'], 'مؤكد')
                        st.rerun()
                
                if appointment['status'] in ['مجدول', 'مؤكد']:
                    if st.button(f"🏁", key=f"complete_{appointment['id']}", help="إكمال"):
                        crud.update_appointment_status(appointment['id'], 'مكتمل')
                        st.rerun()
...
def edit_appointment(appointment_id):
    ...
    # هنا يمكن إضافة نموذج تعديل الموعد
    st.info("سيتم إضافة وظيفة تعديل المواعيد قريباً")
