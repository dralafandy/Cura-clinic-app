        ...
        with st.form("book_appointment_form"):
            ...
            with col1:
                ...
                if selected_patient_id:
                    ...
                    st.info(f"عمر المريض: {format_date_arabic(patient['date_of_birth'])}")
            ...
            if submitted:
                if not (selected_patient_id and selected_doctor_id and selected_treatment_id and appointment_date and appointment_time):
                    ...
                    # ربط بالماليات: إضافة دفعة معلقة إذا لزم
                    crud.create_payment(
...
def show_doctors_appointments_report(appointments_df):
    """تقرير مواعيد الأطباء (ربط بالأطباء)"""
    ...
    doctor_stats.columns = ['عدد المواعيد', 'إجمالي الإيرادات', 'مكتملة']
    doctor_stats = doctor_stats.reset_index()
    doctor_stats.columns = ['اسم الطبيب', 'عدد المواعيد', 'إجمالي الإيرادات', 'مكتملة']
...
def show_patients_appointments_report(appointments_df):
    """تقرير مواعيد المرضى (ربط بالمرضى)"""
...
def appointment_accounts():
    """حسابات المواعيد - ربط بالماليات والمريض والطبيب"""
...
def edit_appointment(appointment_id):
    ...
    appt = crud.get_appointment_by_id(appointment_id)  # أضف هذه الدالة في crud.py
    if appt:
        with st.form("edit_appointment_form"):
            # حقول مشابهة لحجز جديد، مع قيم افتراضية
            # ... (يمكن نسخ من book_new_appointment مع قيم appt)
            submitted = st.form_submit_button("💾 حفظ التعديلات")
            if submitted:
                # تحديث باستخدام crud.update_appointment
                show_success_message("تم تعديل الموعد بنجاح")

def delete_appointment(appointment_id):
    """حذف موعد"""
    crud.delete_appointment(appointment_id)  # أضف في crud.py
