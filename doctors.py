...
try:
    ...
    for _, doctor in doctors_df.iterrows():
        # التصحيح هنا
        doctor_stats = performance_stats[
            performance_stats['اسم الطبيب'].astype(str) == str(doctor['name'])
        ]
        
        doctor_revenue = doctor_stats['إجمالي الإيرادات'].sum() if not doctor_stats.empty else 0
    ...
    for _, doctor in doctors_df.iterrows():
        ...
        if not appointments_df.empty:
            ...
            doctor_appointments = appointments_df[
                appointments_df['doctor_name'].astype(str) == str(doctor['name'])
            ].copy()
